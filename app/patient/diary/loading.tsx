export default function DiaryLoading() {
  return (
    <main className="grid gap-6 p-4 sm:p-6 lg:p-8">
      <div className="grid gap-3">
        <div className="h-4 w-28 animate-pulse rounded-full bg-[color:var(--ui-border)]" />
        <div className="h-10 w-44 animate-pulse rounded-full bg-[color:var(--ui-border)]" />
        <div className="h-4 w-full max-w-2xl animate-pulse rounded-full bg-[color:var(--ui-border)]" />
      </div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="h-32 animate-pulse rounded-[1.5rem] border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]" />
        <div className="h-32 animate-pulse rounded-[1.5rem] border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]" />
        <div className="h-32 animate-pulse rounded-[1.5rem] border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]" />
        <div className="h-32 animate-pulse rounded-[1.5rem] border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]" />
      </div>

      <div className="h-[28rem] animate-pulse rounded-[2rem] border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]" />
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="h-[34rem] animate-pulse rounded-[2rem] border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]" />
        <div className="h-[34rem] animate-pulse rounded-[2rem] border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]" />
      </div>
    </main>
  );
}
